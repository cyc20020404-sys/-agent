...
| Short-Term Memory | `memory/short_term.py` | Redis（回退内存） | 30min | 对话上下文 |
| Long-Term Memory | `memory/long_term.py` | FAISS 磁盘 | 持久 | 知识库文档索引 |
...
| Short-Term Memory | RAM | Redis List，TTL 30min |
| Long-Term Memory | 硬盘 | FAISS 向量索引，磁盘持久化 |
...
### 9.3 一句话总结（适用于简历 Bullet Point）

> **智能客服多 Agent 系统**：基于 LangGraph + FastAPI 构建 Supervisor 编排的 4-Agent 协作架构，自研 MCP 工具协议桥接 LLM 与 Spring Boot 业务 API，实现订单查询/操作、知识库 RAG 问答、合规审查等端到端闭环。支持 admin/user 双角色权限隔离、三层记忆架构降级、OpenTelemetry 全链路追踪。前端基于 Uniapp 构建跨平台 H5 消费者端 + Vue 管理后台。
