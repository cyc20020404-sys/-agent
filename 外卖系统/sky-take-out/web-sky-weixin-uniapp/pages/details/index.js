...
export default {
	...
	methods: {
		...
		goChat() {
			const orderId = this.orderDetailsData && this.orderDetailsData.id
			const prefill = orderId ? '帮我查一下订单 #' + orderId : ''
			uni.navigateTo({
				url: '/pages/chat/chat' + (prefill ? '?prefill=' + encodeURIComponent(prefill) : '')
			})
		},
	}
}